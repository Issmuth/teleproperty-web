  ...
  return (
    ...
    >
      <div className="space-y-5 px-2 lg:px-0">
        ...
        {/* Demo Notice */}
        <div 
          className="min-h-[48px] md:min-h-[52px] bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-2xl md:rounded-3xl px-4 flex items-center gap-2"
          role="status"
          aria-live="polite"
        >
          <Info size={16} className="text-green-600 dark:text-green-500 flex-shrink-0 md:w-5 md:h-5" strokeWidth={2.5} aria-hidden="true" />
        ...
        {/* OTP Input */}
        <form aria-label="OTP verification form">
          <fieldset>
            <legend className="sr-only">Enter 4-digit verification code</legend>
            <div className="flex justify-center gap-2.5 md:gap-3 mt-8" role="group" aria-label="OTP code input">
          ...
          <button
            ...
            onClick={(e) => {
              e.preventDefault();
              router.push(`/auth/role?redirectTo=${encodeURIComponent(redirectTo)}`);
            }}
            disabled={isContinueDisabled}
            aria-label="Verify code and continue to role selection"
